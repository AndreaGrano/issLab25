...
//		System.out.println("RobotState CREATEDDDDDDDDDDDDDDDD " + "x="+x + "y="+y + "direction=" + direction);
	...
	public void setRobotState(int px, int py, Direction dir) {
		x = px;
		y = py;
		direction = dir;
	}
	...
	public RobotState forward() {
		RobotState result = new RobotState(this.x, this.y, this.direction);
		switch(result.direction) {
			case UP: result.x--; break;
			case DOWN: result.x++; break;
			case LEFT: result.y--; break;
			case RIGHT: result.y++; break;
